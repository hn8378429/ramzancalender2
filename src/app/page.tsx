...
const RamzanCalendar = () => {
  ...
  // ✅ FIXED: Helper function to parse date string properly
  const parseDateString = (dateStr: string): Date => {
    // Remove weekday (e.g., "Wednesday, ")
    const cleanDate = dateStr.split(', ')[1];
    // Return date with time set to 00:00:00 local time
    return new Date(cleanDate + ' 00:00:00');
  };

  // ✅ FIXED: calculateTimeRemaining function with proper date parsing
  const calculateTimeRemaining = useCallback(() => {
    ...
    if (!foundSchedule) {
      ...
      // ✅ FIX: Use parseDateString instead of new Date(firstDayStr)
      const firstDay = parseDateString(firstDayStr);
      
      // Parse first Sehri time
      const firstDaySchedule = currentTimetable[0];
      
      const parseTimeForDate = (timeStr: string, baseDate: Date): Date => {
      ...
      const firstSehriTime = parseTimeForDate(firstDaySchedule.sehri, firstDay);
      
      // ✅ FIXED: Show Sehri timer if current time is before first Sehri
      // This includes the entire day/night before Ramzan
      if (now < firstSehriTime) {
        setRamzanStarted(true);
        
        const diffToSehri = firstSehriTime.getTime() - now.getTime();
        const hours = Math.floor(diffToSehri / (1000 * 60 * 60));
        const minutes = Math.floor((diffToSehri % (1000 * 60 * 60)) / (1000 * 60));
        const seconds = Math.floor((diffToSehri % (1000 * 60)) / 1000);
        
        setTimeToSehri(`${hours}h ${minutes}m ${seconds}s`);
        setTimeToIftar('');
      ...
      const diffToStart = firstDay.getTime() - now.getTime();
      
      if (diffToStart > 0) {
        setRamzanStarted(false);
        const diffDays = Math.floor(diffToStart / (1000 * 60 * 60 * 24));
        const diffHours = Math.floor((diffToStart % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
        const diffMinutes = Math.floor((diffToStart % (1000 * 60 * 60)) / (1000 * 60));
