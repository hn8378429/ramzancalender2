...
const firstRamzanDays: Record<string, string> = {
  ...
  'Karachi': 'Thursday, February 19, 2026',
...
const RamzanCalendar = () => {
  ...
  const get2026ManualTimetable = useCallback((city: string): RamzanDate[] => {
    ...
    const lahoreTimetable: RamzanDate[] = karachiTimetable.map(item => ({
      ...item,
      sehri: item.sehri,
      iftar: item.iftar
    }));

    const faisalabadTimetable: RamzanDate[] = karachiTimetable.map(item => ({
      ...item,
      sehri: item.sehri,
      iftar: item.iftar
    }));
    ...
    switch(city) {
      ...
      case 'Lahore': return lahoreTimetable;
      case 'Faisalabad': return faisalabadTimetable;
      case 'Coventry': return coventryTimetable;
      case 'Scunthorpe': return scunthorpeTimetable;
      case 'London': return londonTimetable;
      case 'Birmingham': return birminghamTimetable;
